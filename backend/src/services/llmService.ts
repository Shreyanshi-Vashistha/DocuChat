...
- Include specific details like numbers, timeframes, and requirements when available in the context`;
  ...
  private enhancedFallbackResponse(
    question: string,
    contextWithMeta: ContextWithMetadata[]
  ): LLMResponse {
    if (contextWithMeta.length === 0) {
      return {
        answer:
          "I couldn't find any relevant information in the company policy document to answer your question. Please try asking about topics covered in the employee handbook, such as vacation policies, benefits, working hours, or performance reviews.",
        sources: [],
      };
    }
    ...
    const scoredContexts = contextWithMeta.map((ctx) => {
      ...
      if (
        ctx.section &&
        questionWords.some((word) => ctx.section!.toLowerCase().includes(word))
      ) {
        score += 3;
      }
    ...
    if (bestMatch.score > 1) {
      const sentences = bestMatch.content
        .split(/[.!?]+/)
      ...
      if (relevantSentences.length > 0) {
        const sectionPrefix = bestMatch.section
          ? `According to the ${bestMatch.section} section: `
          : "Based on the company policy: ";
        answer = sectionPrefix + relevantSentences.join(". ").trim() + ".";

        if (relevantSentences.length < sentences.length) {
          const additionalContext = sentences
            .filter(
              (s) => !relevantSentences.includes(s) && s.trim().length > 15
            )
            .slice(0, 2);

          if (additionalContext.length > 0) {
            answer +=
              " Additionally: " + additionalContext.join(". ").trim() + ".";
          }
        }
      } else {
        answer = `Based on the company policy document: ${bestMatch.content.substring(
          0,
          300
        )}...`;
      }
