...
export class VectorService {
  ...
  private buildSectionKeywords(): void {
    ...
    this.sectionKeywords.set("COMPANY OVERVIEW", [
      "company",
      "docuchat",
      "mission",
      "technology",
    ...
    this.sectionKeywords.set("WORKING HOURS AND TIME OFF", [
      "hours",
      "working",
      "time",
      "vacation",
    ...
    this.sectionKeywords.set("BENEFITS PACKAGE", [
      "benefits",
      "health",
      "insurance",
      "retirement",
    ...
    this.sectionKeywords.set("CODE OF CONDUCT", [
      "conduct",
      "behavior",
      "ethics",
      "harassment",
    ...
    this.sectionKeywords.set("REMOTE WORK POLICY", [
      "remote",
      "work",
      "home",
      "flexible",
    ...
    this.sectionKeywords.set("PERFORMANCE REVIEWS", [
      "performance",
      "review",
      "evaluation",
      "annual",
    ...
    this.sectionKeywords.set("TECHNOLOGY AND SECURITY", [
      "technology",
      "security",
      "equipment",
      "laptop",
    ...
    this.sectionKeywords.set("EXPENSE REIMBURSEMENT", [
      "expense",
      "reimbursement",
      "travel",
      "business",
    ...
    this.sectionKeywords.set("COMMUNICATION POLICIES", [
      "communication",
      "email",
      "slack",
      "messaging",
    ...
    this.sectionKeywords.set("TERMINATION PROCEDURES", [
      "termination",
      "resignation",
      "employment",
      "notice",
  ...
  private calculateSemanticSimilarity(query: string, text: string): number {
    ...
    const semanticGroups = new Map([
      ...
      [
        "work",
        ["job", "employment", "career", "position", "role", "working", "hours"],
      ],
      ["remote", ["home", "telecommute", "virtual", "distance", "flexible"]],
      [
        "policy",
        ["rule", "regulation", "procedure", "guideline", "requirement"],
      ],
      ...
      [
        "training",
        ["education", "learning", "development", "course", "professional"],
      ],
      ...
      [
        "technology",
        ["equipment", "laptop", "computer", "software", "security"],
      ],
    ...
    queryWords.forEach((queryWord) => {
      if (textWords.includes(queryWord)) {
        ...
        const hasRelated = textWords.some(
          (textWord) =>
    ...
    return queryWords.length > 0
      ? Math.min(1.0, semanticScore / queryWords.length)
      : 0;
  }
  ...
  private calculateSectionSimilarity(
    ...
    const directMatch =
      queryWords.filter((word) => sectionWords.includes(word)).length /
      Math.max(queryWords.length, 1);

    const sectionKeywords = this.sectionKeywords.get(chunkSection) || [];
    const keywordMatch =
      queryWords.filter((word) => sectionKeywords.includes(word)).length /
      Math.max(queryWords.length, 1);
